...
if($_SERVER['REQUEST_METHOD']=='POST' &&
	...
	//拡張子から画像ファイルだと分かったとき
	if($extension == "pdf"){
		$sql = sprintf("INSERT INTO bbs_data(Comment_number,User,Comment,Time,Password,File_type,File_path) VALUES (%d,%s,%s,%s,%s,%s,%s)",quote_smart($Comment_number),quote_smart($user),quote_smart($message),quote_smart($postedAt),quote_smart($password),quote_smart("画像"),quote_smart("/home/co-67.3919.com/public_html/kadai3/files/" . $_FILES["upfile"]["name"]));
		echo "通っている。";
	}else if($extension == "wmv"|"avi"|"mpeg"|"divx"|"rm"|"mpg"){
		$sql = sprintf("INSERT INTO bbs_data(Comment_number,User,Comment,Time,Password,File_type,File_path) VALUES(%d,%s,%s,%s,%s,%s,%s)",quote_smart($Comment_number),quote_smart($user),quote_smart($message),quote_smart($postedAt),quote_smart($password),quote_smart("動画"),quote_smart("/home/co-67.3919.com/public_html/kadai3/files/" . $_FILES["upfile"]["name"]));
	}
	$result_flag = mysql_query($sql);
	if(!$result_flag){
