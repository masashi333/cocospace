...
function smarty($data_count,$queryset){
	...
	//文字をテンプレートに渡す
	$smarty->assign('data_count',$data_count,ture);
	$smarty->assign('queryset',$queryset,ture);
	$smarty->display('cache.tpl',$my_cache_id);

}
function insert_getkeiziban($params,&$smarty)
{

	return $smarty->fetch('display.tpl');
}
...
if( $cache_data=$cache->get($cache_id) ){
    ...
    // データ取得処理ここから
      $url = "http://co-67.3919.com/kadai4/Smarty_program/cache/cache_display.php";
      //Webデータ取得先アドレス
